    ...
    * @param eventIdAirDropChallenge a tracking id used to sync db
    ...
    function airdropChallenge(
        ...
        uint256 eventIdAirDropChallenge
            ...
            uint256 airdropFee = uint256(BULLZ_FEE).mul(amount);
            require(MARKET_TOKEN.balanceOf(_msgSender()) >=  airdropFee, "Challenge Exchange: Insufficient balance");
            ...
            emit AirDropChallenge(challengeId, receiver, amount, airdropFee,  eventIdAirDropChallenge);
            bool success = MARKET_TOKEN.transferFrom(_msgSender(), owner(), airdropFee);
    ...
    function addTokenChallenge(        
        ...
        uint256 eventIdAddTokenChallenge)external override returns (bytes32){
            ...
            uint256 escrowAmount = _getEscrowAmount(token, tokenAmount, winnerCount);
            require(challengeToken.balanceOf(_msgSender()) >= escrowAmount , "Challenge Exchange: Insufficient balance");
            require(airdropStartAt > block.timestamp, "Challenge Exchange: invalid start at airdrop");
            require(airdropEndAt > airdropStartAt, "Challenge Exchange: invalid start at airdrop");
            ...
            bool success = challengeToken.transferFrom(_msgSender(), address(this), escrowAmount);
            require(success, "Challenge Exchange: Token transfer did not succeeded ");            
            return challengeId;
    }
    ...
    function airdropTokenChallenge(
        ...
        uint256 eventIdAirDropTokenChallenge
            ...
            challenge.winnerCount = challenge.winnerCount.sub(1); 

            bool success = challengeToken.transfer(owner(), tokenAirdropFee);
            require(success, "Challenge Exchange: Airdrop fee transfer did not succeeded ");

            bool success1 = challengeToken.transfer(receiver, challenge.tokenAmount);
            require(success1, "Challenge Exchange: Airdrop transfer did not succeeded ");
            
            emit AirDropTokenChallenge(challengeId, receiver, challenge.tokenAmount, tokenAirdropFee, eventIdAirDropTokenChallenge);
            return true;
    }
    ...
    function withdrawTokenChallenge(bytes32 challengeId, uint256 eventIdWithdrawTokenChallenge)external  override returns (bool){
        ...
        IERC20 challengeToken = IERC20(challenge.token);
        bool success = challengeToken.transfer(_msgSender(), _getEscrowAmount(challenge.token, challenge.tokenAmount, challenge.winnerCount));
            require(success, "Challenge Exchange: Token transfer did not succeeded ");   
    ...
    function _getEscrowAmount(address token, uint256 tokenAmount, uint256 winnerCount) internal view returns (uint256){
        uint256 tokenAirdropFee = _getTokenAirdropFee(token, tokenAmount);

        return winnerCount.mul(tokenAmount.add(tokenAirdropFee));
    }
    ...
    function setPrimaryToken(address token) onlyOwner() external  override returns (bool){
        require(token != address(0), "Challenge Exchange: Not a valid address");
        PRIMARY_TOKEN = token;
